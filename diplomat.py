...
class Diplomat(commands.Cog):
    def __init__(self):
        pass

    @commands.command(name='vouch', help='Diplomat only, gives member of the guild leader\' alliance the ability to use flamesofexile.com')
    async def diplo_vouch(self, context, user=None):
    ...
    @commands.command(name='unvouch', help='Diplomat only, removes permissions from member of diplomat\'s guild')
    async def diplo_endvouch(self, context, user=None):
        diplo = context.author
        if roles.diplo_role not in context.author.roles and roles.admin_role not in context.author.roles:
            await context.send('you do not have the required permissions to invoke this action')

...
async def vouch(context, roles, auth_token, user):
    ...
    if roles.diplomat_role not in context.author.roles or roles.admin_role not in context.author.roles:
    ...
    if response.status_code == 200:
        target_user.add_roles(roles.alliance_role)
...
async def endvouch(context, roles, auth_token, user):
    ...
    if response.status_code == 200:
        target_user.remove_roles(roles.alliance_role)
